import React from 'react';
import { Link } from 'react-router-dom';

const Nebosh = () => {
    return (
        <div>
            <div className='container'>
                <div className='nebosh-header pt-5 py-5'>
                    <h2 className='text-center p-3 rounded h1'>NEBOSH</h2>
                    <div className='row'>
                        <div className='col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 pt-5 py-5'>
                            <h5 className='h2'>NEBOSH IGC</h5>

                            <p className='m-2'>
                                NEBOSH International General Certificate is one
                                of the fastest growing qualification in the
                                field of health and safety. This brings in the
                                enthusiastic demand for this course all over the
                                world. BESI Private Limited is the leading body
                                to give NEBOSH IGC training in and from
                                Bangladesh, to trainees worldwide. Every
                                session, BESI registers for candidates from
                                Dubai, Singapore, Qatar, Oman, India, Pakistan
                                and Bangladesh
                            </p>
                        </div>
                    </div>

                    <div className='row m-2'>
                        <div className='col-xl-6 col-lg-6 col-md-6 col-sm-12 col-xs-12 border py-3 pt-3 rounded'>
                            <div className='single-information'>
                                <h4>NEBOSH IGC</h4>
                                <p>
                                    The NEBOSH International General Certificate
                                    in Occupational Health & Safety Training
                                    course is done to implement safety system.
                                    This course is based on the UK’s world
                                    leading standard. NEBOSH IGC is one of the
                                    most recommended courses for safety
                                    professionals. More than 90,000 people
                                    around the world are holding this
                                    prestigious certificate.
                                </p>
                                <br />
                                {/* 02 */}
                                <div className='details-info'>
                                    <h5>Aim or Purpose(why this course)</h5>
                                    <div className='main-list'>
                                        <ul className='list-group'>
                                            <li className='list-group-item'>
                                                • Leading and most recognized
                                                safety course.
                                            </li>
                                            <li className='list-group-item'>
                                                • Most demanding course.
                                            </li>
                                            <li className='list-group-item'>
                                                • This course is the foundation
                                                for a secure career in Health &
                                                Safety.
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                {/* 02 */}
                                <div className='course-content pt-3'>
                                    <h5>Who can attend?</h5>
                                    <p>
                                        Anyone with better English skills can
                                        join this course (Fresher, Manager,
                                        Supervisor and anyone related to health
                                        & safety background.)
                                    </p>
                                    <h5>Course Content</h5>
                                    {/* Unit-01 */}
                                    <p>
                                        <strong>Unit IG1:</strong> Management of
                                        Health & Safety (Theory Paper)
                                    </p>

                                    <div className='main-list'>
                                        <ul className='list-group'>
                                            <li className='list-group-item'>
                                                • Why we should manage workplace
                                                health & safety
                                            </li>
                                            <li className='list-group-item'>
                                                • How health and safety
                                                management systems work and how
                                                they look like
                                            </li>
                                            <li className='list-group-item'>
                                                • Managing risk- understanding
                                                people and processes.
                                            </li>
                                            <li className='list-group-item'>
                                                • Health & safety monitoring and
                                                measuring unit
                                            </li>
                                        </ul>
                                    </div>
                                    {/* Unit-02 */}

                                    <div className='main-list pt-3'>
                                        <p>
                                            <strong>Unit IG2:</strong> Risk
                                            Assessment (Practical Assessment)
                                        </p>
                                        <ul className='list-group'>
                                            <li className='list-group-item'>
                                                • Physical and psychological
                                                health
                                            </li>
                                            <li className='list-group-item'>
                                                • Musculoskeletal health
                                            </li>
                                            <li className='list-group-item'>
                                                • General workplace issues
                                            </li>
                                            <li className='list-group-item'>
                                                • Work equipment
                                            </li>
                                            <li className='list-group-item'>
                                                • Fire
                                            </li>
                                            <li className='list-group-item'>
                                                • Electricity
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>

                        {/* Col-6 */}

                        <div className='col-xl-6 col-lg-6 col-md-6 col-sm-12 col-xs-12 border py-3 pt-3 rounded'>
                            <div className='single-information'>
                                <h4>Course Requirements</h4>
                                <p>English language skill is preferred</p>
                                <br />
                                {/* 02 */}
                                <div className='details-info'>
                                    <h5>Objectives</h5>
                                    <p>
                                        After completing the course, applicant
                                        will be able to:
                                    </p>
                                    <div className='main-list'>
                                        <ul className='list-group'>
                                            <li className='list-group-item'>
                                                • Identify and control workplace
                                                hazards
                                            </li>
                                            <li className='list-group-item'>
                                                • Ensure organizations’ safety
                                                standards and legislation
                                            </li>
                                            <li className='list-group-item'>
                                                • Meet the academic requirements
                                                for TECH IOSH, AIOSH, and AIIRSM
                                                membership
                                            </li>
                                            <li className='list-group-item'>
                                                • Pursue a strong & secure
                                                career in health & safety
                                            </li>
                                            <li className='list-group-item'>
                                                • Gain a strong foundation to
                                                achieve the next level
                                                certificate “NEBOSH
                                                International Diploma”.
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                {/* 02 */}
                                <div className='course-content pt-3'>
                                    <h5>Assessment</h5>
                                    {/* Unit-01 */}
                                    <p>
                                        <strong>Unit IG1:</strong> is assessed
                                        by a 24 hours open book exam.
                                    </p>

                                    <div className='main-list'>
                                        <ul className='list-group'>
                                            <li className='list-group-item'>
                                                • Pass mark: 45
                                            </li>
                                            <li className='list-group-item'>
                                                • Online template
                                            </li>
                                        </ul>
                                    </div>
                                    {/* Unit-02 */}

                                    <div className='main-list pt-3'>
                                        <p>
                                            <strong>Unit IG2:</strong> is
                                            assessed by a practical assessment.
                                        </p>
                                        <ul className='list-group'>
                                            <li className='list-group-item'>
                                                • Electronic submission form
                                            </li>
                                            <li className='list-group-item'>
                                                • No specific Pass/Fail marks
                                            </li>
                                            <li className='list-group-item'>
                                                • Result will be either pass or
                                                fail
                                            </li>
                                        </ul>
                                    </div>
                                    <p className='pt-2'>
                                        <strong>
                                            A MOC Test / Closing Interview will
                                            be taken (360 Degrees).
                                        </strong>
                                    </p>

                                    <div className='main-list'>
                                        <h5>Why BESI?</h5>
                                        <ul className='list-group'>
                                            <li className='list-group-item'>
                                                • BESI Private Limited is NEBOSH
                                                Solutions Accredited.
                                            </li>
                                            <li className='list-group-item'>
                                                • Leading safety training
                                                provider in Bangladesh.
                                            </li>
                                            <li className='list-group-item'>
                                                • Customized course materials
                                                for better lessons.
                                            </li>
                                            <li className='list-group-item'>
                                                • Well trained and highly
                                                experienced NEBOSH approved
                                                trainers.
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='row m-2'>
                        <div className='col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 pt-5 py-5'>
                            <h5 className=''>Course Duration</h5>

                            <div className='main-list'>
                                <ul className='list-group'>
                                    <li className='list-group-item'>
                                        • 18 classes ( additional classes)
                                    </li>
                                </ul>
                            </div>
                            <h5 className=''>Course Fee:</h5>

                            <div className='main-list'>
                                <ul className='list-group'>
                                    <li className='list-group-item'>
                                        Regular fee: 65,000 BDT (approx. USD
                                        765)
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <div className='button-div '>
                            <Link to=''>
                                <button className='btn btn-main btn-lg btn-block'>
                                    Pay Course
                                </button>
                            </Link>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Nebosh;
